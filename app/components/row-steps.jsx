...
const RowSteps = React.forwardRef(
  (
    ...
    return (
      <nav
        aria-label="Progress"
        className="-my-4 w-full justify-center flex overflow-x-auto py-4"
      >
        <ol
          className={cn("flex max-w-[300px] justify-between w-full flex-row flex-nowrap gap-x-3", colors, className)}
        >
          {steps?.map((step, stepIdx) => {
            ...
            return (
              <li key={stepIdx} className="relative flex flex-col items-center w-[78px]">
                <button
                  key={stepIdx}
                  ref={ref}
                  aria-current={status === "active" ? "step" : undefined}
                  className={cn(
                    "group flex flex-col items-center justify-center gap-y-2 rounded-large py-2.5",
                    stepClassName
                  )}
